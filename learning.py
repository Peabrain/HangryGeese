...
#    x = tf.transpose(x, [0, 2, 3, 1])
    x = keras.layers.Conv3D(filters=64, kernel_size=(1,3,3), padding='same',activation='linear')(x)#,return_sequences=False)(x)
    x = keras.layers.MaxPool3D((1,2,2))(x)
    x = keras.layers.Conv3D(filters=64, kernel_size=(1,3,3), padding='same',activation='linear')(x)#,return_sequences=False)(x)
    x = keras.layers.MaxPool3D((1,2,2))(x)
    x = keras.layers.Conv3D(filters=64, kernel_size=(1,2,2), padding='valid',activation='linear')(x)#,return_sequences=False)(x)
#    x = keras.layers.Reshape((memorySize,32))(x)
#    print(x.shape)
        ...
        for i in range(len(actions_rel)):
            if len(self.player[i]) > 0:
                ...
                if self.player[i][0] in new_player_set:
                    reward[i] = -2 # len(self.player[i])#(self.round * 2 + len(self.player[i]))
                    killed = True
                    dones[i] = True
                    self.player[i] = []
...
#        for i in range(new_food):
        ...
        for i in range(len(self.player)):
            if not reward[i] is None and (reward[i] == -1 or reward[i] >= 1):
                ...
                pl_count_idx = i
        if pl_count == 1:
            winner = self.round * 2 + len(self.player[pl_count_idx])
#            reward[pl_count_idx] += 2 # self.round * 2 + len(self.player[pl_count_idx])
            dones[pl_count_idx] = True

        return reward, dones, winner
...
#                print(game.MemorySteps[i])
        ...
        reward, dones, winner = game.do_action(action)
        done_count = 0
        for i in range(len(game.player)):
...
#                game_state  = ([game.player[(i + j) % len(game.player)] for j in range(len(game.player))],game.food,game.last_action[i])
    ...
    return np.array(rewards,dtype=np.int32),game.round,winner
...
#        memory.pop()
        ...
        if (k + 1) % 50 == 0:
            ...
            for i in range(1):
                ...
                re,ro,wi = play_game(0,model_target,memory)
                rewards += re
                rounds += ro
                winners += wi
            print(' test: ',k // 50,rewards,np.max(rewards),np.sum(rewards),rounds,winners,eps)
